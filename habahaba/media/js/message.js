...
function handleMessage(aJSJaCPacket) {
    ...
    if(isAnonymousMode) {
        messageFromJID = JIDUnQuote(messageFromJID);
    }
    ...
    if(!handleError(messageNode)) {
        ...
        var messageFromJIDNoResource = cutResource(messageFromXHashable);
        
        // We generate the hash
        var messageFromJIDHashable = '' + messageFromJIDNoResource + '';
        var fromJIDHash = hex_md5(messageFromJIDHashable);
        var noHashfriendJID = messageFromJIDNoResource;
        ...
        // Normal messages
            ...
            var messageID = hex_md5(messageFromJIDNoResource + messageSubject + dateOfTheMessage);
            var messageDate = getXMPPTime();
            
            // We display the received message
            displayMessage(messageFromJIDNoResource, messageSubject, messageBody, 'unread', messageID, 'fresh', messageDate);
        }
